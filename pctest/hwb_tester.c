...
int main(int argc, char **argv)
{
	...
	fd_serial = openSerial(argv[1], O_RDWR);
	if (fd_serial == -1)
	{
	...
	options.c_lflag &= ~(ECHOK | ECHONL); // disable more stuff
	//options.c_iflag &= ~(IXON | IXOFF | IXANY); // no software flow control
	options.c_iflag &= ~(IXON | IXOFF); // no software flow control
	//options.c_iflag &= ~(INPCK | INLCR | IGNCR | ICRNL | IUCLC); // disable more stuff
	options.c_iflag &= ~(INPCK | INLCR | IGNCR | ICRNL); // disable more stuff
