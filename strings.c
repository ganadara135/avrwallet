...
/** Permission denied (host canceled action) string. */
static const char str_MISCSTR_PERMISSION_DENIED_HOST[] PROGMEM = "Host canceled action";
...
static const char str_MISCSTR_CONFIG[] PROGMEM = "Config Message";
...
static const char str_WALLET_NOT_LOADED[] PROGMEM = "Wallet not юсаж loaded";
...
char getString(StringSet set, uint8_t spec, uint16_t pos)
{
	...
	if (set == STRINGSET_MISC)
	{
		...
		case MISCSTR_VENDOR: 
			...
			return (char)LOOKUP_BYTE(str_MISCSTR_VERSION[pos]);
			break;
		case MISCSTR_PERMISSION_DENIED_USER:
			return (char)LOOKUP_BYTE(str_MISCSTR_PERMISSION_DENIED_USER[pos]);
			break;
		case MISCSTR_INVALID_PACKET:
			return (char)LOOKUP_BYTE(str_MISCSTR_INVALID_PACKET[pos]);
			break;
		case MISCSTR_PARAM_TOO_LARGE:
			return (char)LOOKUP_BYTE(str_MISCSTR_PARAM_TOO_LARGE[pos]);
			break;
		case MISCSTR_PERMISSION_DENIED_HOST:
			return (char)LOOKUP_BYTE(str_MISCSTR_PERMISSION_DENIED_HOST[pos]);
			break;
		case MISCSTR_UNEXPECTED_PACKET:
			return (char)LOOKUP_BYTE(str_MISCSTR_UNEXPECTED_PACKET[pos]);
			break;
		case MISCSTR_OTP_MISMATCH:
			return (char)LOOKUP_BYTE(str_MISCSTR_OTP_MISMATCH[pos]);
			break;
		case MISCSTR_CONFIG:
			return (char)LOOKUP_BYTE(str_MISCSTR_CONFIG[pos]);
			break;
		default:
			return (char)LOOKUP_BYTE(str_UNKNOWN[pos]);
			break;
		}
	}
	else if (set == STRINGSET_WALLET)
	{
		...
		{
		case WALLET_FULL:
			return (char)LOOKUP_BYTE(str_WALLET_FULL[pos]);
			break;
		case WALLET_EMPTY:
			return (char)LOOKUP_BYTE(str_WALLET_EMPTY[pos]);
			break;
		case WALLET_READ_ERROR:
			return (char)LOOKUP_BYTE(str_WALLET_READ_ERROR[pos]);
			break;
		case WALLET_WRITE_ERROR:
			return (char)LOOKUP_BYTE(str_WALLET_WRITE_ERROR[pos]);
			break;
		case WALLET_ADDRESS_NOT_FOUND:
			return (char)LOOKUP_BYTE(str_WALLET_ADDRESS_NOT_FOUND[pos]);
			break;
		...
		case WALLET_NOT_LOADED:
			...
			return (char)LOOKUP_BYTE(str_WALLET_NOT_LOADED[pos]);
			break;
		case WALLET_INVALID_HANDLE:
			return (char)LOOKUP_BYTE(str_WALLET_INVALID_HANDLE[pos]);
			break;
		case WALLET_BACKUP_ERROR:
			return (char)LOOKUP_BYTE(str_WALLET_BACKUP_ERROR[pos]);
			break;
		case WALLET_RNG_FAILURE:
			return (char)LOOKUP_BYTE(str_WALLET_RNG_FAILURE[pos]);
			break;
		case WALLET_INVALID_WALLET_NUM:
			return (char)LOOKUP_BYTE(str_WALLET_INVALID_WALLET_NUM[pos]);
			break;
		case WALLET_INVALID_OPERATION:
			return (char)LOOKUP_BYTE(str_WALLET_INVALID_OPERATION[pos]);
			break;
		default:
			return (char)LOOKUP_BYTE(str_UNKNOWN[pos]);
			break;
	...
	else if (set == STRINGSET_TRANSACTION)
	{
		...
		case TRANSACTION_INVALID_FORMAT:
			return (char)LOOKUP_BYTE(str_TRANSACTION_INVALID_FORMAT[pos]);
			break;
		case TRANSACTION_TOO_MANY_INPUTS:
			return (char)LOOKUP_BYTE(str_TRANSACTION_TOO_MANY_INPUTS[pos]);
			break;
		case TRANSACTION_TOO_MANY_OUTPUTS:
			return (char)LOOKUP_BYTE(str_TRANSACTION_TOO_MANY_OUTPUTS[pos]);
			break;
		case TRANSACTION_TOO_LARGE:
			return (char)LOOKUP_BYTE(str_TRANSACTION_TOO_LARGE[pos]);
			break;
		case TRANSACTION_NON_STANDARD:
			return (char)LOOKUP_BYTE(str_TRANSACTION_NON_STANDARD[pos]);
			break;
		case TRANSACTION_INVALID_AMOUNT:
			return (char)LOOKUP_BYTE(str_TRANSACTION_INVALID_AMOUNT[pos]);
			break;
		case TRANSACTION_INVALID_REFERENCE:
			return (char)LOOKUP_BYTE(str_TRANSACTION_INVALID_REFERENCE[pos]);
			break;
		default:
			return (char)LOOKUP_BYTE(str_UNKNOWN[pos]);
			break;
